...
`prove-it`는 그 보고를 검사로 바꿉니다. 레포 루트에 `verify.sh`를 두세요. 에이전트가 자기 차례를 끝내려고 하면 훅이 스크립트를 실행하고, 종료 코드가 0이 아니면 에이전트를 멈추게 두는 대신 다시 일하도록 돌려보냅니다.

![완료됐다고 주장하는 에이전트를 prove-it가 막는 모습](../../docs/demo.svg)

게이트는 한 차례에 최대 세 번까지 되돌려 보내고 그다음에는 양보합니다. 절대 양보하지 않는 훅은 세션을 멈춰 세우기 때문입니다. 양보하는 것과 통과하는 것은 같지 않으므로, 당신이 마지막으로 보게 되는 것은 "완료"라는 말이 아니라 차례가 검증되지 않은 채 끝났다는 경고입니다. 셋은 당신이 바꿀 수 있는 숫자이고, 이 중 어느 것도 당신의 에이전트가 게이트를 통과할 수 없다는 주장은 아닙니다. 게이트를 조용히 지나갈 수는 없다는 주장입니다.

레포에 에이전트가 일을 돌려주기 전에 반드시 참이어야 하는 로컬 명령이 있을 때 쓰세요. 테스트, 타입 검사, lint, 생성 파일 검사, 마이그레이션 dry run, 버그가 사라졌음을 보여주는 작은 smoke test가 여기에 들어갑니다. `prove-it`이 가장 쓸모 있는 곳은 에이전트가 코드를 고치고 같은 대화에서 "완료"라고 말하는 레포입니다.

sandbox, CI 대체재, 긴 네트워크 작업을 넣는 장소로 쓰지는 마세요. 검사가 secrets나 production 접근이 필요하거나 1분쯤 넘게 걸린다면 그 검사는 CI에 두고, `verify.sh`에는 에이전트가 아직 작업 중일 때 로컬에서 실행할 수 있는 증거만 남기세요.

첫날 흐름은 작게 시작합니다. 플러그인을 설치하고 `/prove-it:init`을 실행한 뒤, 생성된 `git diff --check`만 활성 검사로 둡니다. 그다음 실제 명령 하나를 손으로 직접 통과하는 걸 본 뒤 켭니다. 이후 에이전트가 레포를 바꾸고 멈추려 하면, `verify.sh`가 작업을 돌려줘도 되는지 결정합니다.
...
"변경했다"는 어떤 도구가 돌았는지의 기록이 아니라 레포가 답합니다. 세션이 시작될 때 훅은 트리가 어떤 모습이었는지 기록하고, 멈출 때마다 트리가 여전히 그 모습인지 묻습니다. `sed`로 다시 쓴 파일, `git apply`로 적용한 패치, 코드 생성기가 뱉어낸 파일, 그리고 커밋은 모두 변경입니다. 넷 다 트리를 바꾸기 때문입니다. 읽기만 한 세션은 열릴 때 이미 더러웠던 레포에서조차 아무것도 아닌 것으로 칩니다.

마지막 조건은 통과한 트리를 멈출 때마다가 아니라 한 번만 검증한다는 뜻입니다. 검증이 실패하면 에이전트는 출력의 마지막 스무 줄을 보게 되는데, 대개 이 정도면 무엇이 잘못됐는지 따로 알려주지 않아도 원인을 고칩니다.

`PROVE_IT_SKIP=1`은 일부러 게이트를 통과합니다. 세션과 세션 사이에 `verify.sh`를 삭제하면 게이트가 완전히 꺼집니다. 두 탈출구 모두 의도한 것입니다. 사람들은 없앨 수 없는 게이트는 돌아서 지나가기 마련이니까요. `PROVE_IT_MAX_BLOCKS`는 한 차례가 몇 번이나 되돌려 보내질 수 있는지를 정하고, `0`은 게이트가 절대 막지 않으면서 보고만 하게 합니다.

## 에이전트가 게이트를 편집할 때

가장 다루기 힘든 실패 양상은 불안정한 검사가 아닙니다. `verify.sh`를 통과시키지 못하자 대신 `verify.sh`를 편집해버리는 에이전트입니다.

그중 가장 손쉬운 형태는 게이트를 아예 무장 해제하는 것이라, 게이트는 그걸 거부합니다. 훅은 세션이 시작될 때 `verify.sh`가 실행 가능했는지를 기록하고, 그 파일이 삭제되거나 실행 비트가 벗겨진 채로 끝나는 세션은 막고, 무슨 짓을 했는지 알려주고, 그게 정말 의도였다면 정직하게 빠져나가는 법을 알려줍니다. 세션과 세션 사이에 `verify.sh`를 삭제하는 것은 여전히 탈출구이고 여전히 명령 하나면 됩니다.

강제되지 않고 남는 것은 미묘한 형태입니다. `verify.sh`를 실행 가능한 채로 두면서 그 안의 검사를 조용히 도려내는 에이전트 말입니다. 실패 메시지가 그러지 말라고 말하고, [SPEC.md](../../SPEC.md)는 이걸 수정이 아니라 위반이라고 부릅니다. 하지만 둘 중 어느 것도 강제는 아닙니다. 당신의 diff를 직접 읽으세요. 스펙의 diff 증거가 바로 그걸 위해 있는 겁니다.
...
경계가 셋 더 있는데, 그러지 않으면 나쁜 순간에 마주치게 되므로 솔직히 적어 둡니다. 게이트는 `PROVE_IT_MAX_BLOCKS`번의 거부 뒤에 양보하므로, 작정한 에이전트는 결국 차례의 끝에 도달합니다. 다만 그곳에 조용히 도달하지는 못합니다. 당신의 `.gitignore`가 제외한 파일은 변경 감지에 보이지 않으므로, 무시된 `.env`를 읽는 `verify.sh`는 그 파일만 바뀌었을 때 건너뛰어질 수 있습니다. 그리고 나중에 편집하게 될 레포 바깥에서 시작한 세션은 비교할 기준선이 없어서, 게이트는 워킹 트리가 더러운지를 보는 더 약한 검사로 되돌아갑니다.
...
`prove-it`에는 `verify.sh`가 있고, 그 스크립트가 돌리는 것 중 하나는 게이트 자체입니다. 임시 디렉터리 안의 진짜 git 레포를 상대로 돌리죠. 실패하는 검사는 막고, 통과하는 검사는 허용하고, 읽기 전용 세션은 건드리지 않고, 커밋 뒤의 깨끗한 트리를 작업 없음으로 착각하지 않고, 우회는 동작합니다.

```bash
./verify.sh
```

CI는 Linux와 macOS에서 같은 스크립트를 돌리고, 검사가 실패하는 레포를 게이트가 여전히 막는지 증명하는 별도 작업도 함께 돌립니다. 이 레포는 CodeQL, OpenSSF Scorecard, 그리고 소스를 checksum과 GitHub provenance attestation과 함께 패키징하는 태그 release workflow도 돌립니다.

## 프로젝트 신뢰

신뢰하지 않는 레포에서 사용하기 전에 [SECURITY.md](../../SECURITY.md)를 읽으세요. `prove-it`은 레포가 소유한 `verify.sh`를 실행합니다. 이 도구는 guardrail이지 sandbox가 아닙니다.

릴리스 절차는 [RELEASE.md](../../RELEASE.md)에 있습니다. 검증, workflow 상태, checksum, provenance attestation 체크리스트가 포함되어 있습니다. 지원 범위는 [SUPPORT.md](../../SUPPORT.md)에 있습니다.
